...
# Cloud demo-stack LTMS apply:
# 1) mediasync (download)
# 2) if changed -> mediamgr (deploy) -> restart mw container (same compose project)
...
# Use Docker Engine API via unix socket to find mw container for this compose project
project="${DEMO_NAME:-${COMPOSE_PROJECT_NAME:-}}"
if [[ -z "$project" ]]; then
  # fallback: try infer from hostname pattern
  project="storer"
fi
